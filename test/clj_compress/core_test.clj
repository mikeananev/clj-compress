...
(deftest create-archive-test
  ...
  (testing "compress / decompress folder test"
    (let [in-folder  "data/test-folder"
          out-folder "data/out/"]
      (doseq [c (filter #(not= "lz4-framed" %) compressors)] ;; exclude lz4 cause it very very slow on big files
        (let [arch-name (create-archive "test-arch" [in-folder] "data/" c)
              _         (decompress-archive arch-name out-folder c)
              src-count (count (file-seq (file in-folder)))
              out-count (dec (count (file-seq (file out-folder))))]
          ...
          (FileUtils/deleteDirectory (file out-folder))
